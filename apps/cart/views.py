...
def cart_detail(request):
    ...
    if request.POST.get('mainAction') == 'post':
        ...
        ajaxIngredientList = request.POST.get('ingredientList')
        ingredientList=[]
        if ajaxIngredientList:
            ingredientList = json.loads(ajaxIngredientList)
        print(productID, "productID5")
        productQTY = 1
        if subAction == "delete":
        ...
        if subAction == 'add':
            ...
            product_exist = cart.add(product_id=productID, product=product, quantity=productQTY, 
                                ingredientList=ingredientList, instruction=instruction, update_quantity=False)
            response = JsonResponse(
                {'cart_length': cart.__len__()})
            return response

        if subAction == 'update':
            print(productID, "productID6")
            product = get_object_or_404(Product, id=productID)
...
def delivery_and_address(request):
    ...
    if "purchase" in request.session and request.user.is_authenticated:
        ...
        deliveryoptions = DeliveryOptions.objects.filter(is_active=True)
    if "address" in request.session and request.user.is_authenticated:
