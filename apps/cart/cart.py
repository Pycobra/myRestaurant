...
class Cart(object):
    ...
    def add(self, product_id, product, quantity, ingredientList, 
                instruction, update_quantity=False):
        product_id = str(product_id)
        if product_id in self.cart:
            self.cart[product_id]['quantity'] = quantity
            product_exist = True
        if product_id not in self.cart:
            self.cart[product_id] = {'id': product_id, 'quantity':quantity, 'ingredientList':ingredientList, 'total_price':0,
                                     'price':product.price}
            product_exist = False
        self.save()
    ...
    def subtract(self,product_id, quantity=1, update_quantity=False):
        ...
        delivery_price=0
        item_quantity=0
        print(self.cart[product_id]['quantity'], self.cart.__len__(), "self.cart[product_id]['quantity']-4, self.cart.__len__()-4")
        if product_id not in self.cart:
            pass
        if update_quantity:
            self.cart[product_id]['quantity'] -= int(quantity)
            ...
            if self.cart[product_id]['quantity'] == 0:
                self.remove(product_id)
        self.save()
        print(self.cart.__len__(), "self.cart.__len__()-5")
        print(item_quantity, item_total_cost, sub_total, total, delivery_price, 'item_quantity, item_total_cost, sub_total, total, delivery_price')
        return item_quantity, item_total_cost, sub_total, total, delivery_price
